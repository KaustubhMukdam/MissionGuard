...
def render():
    ...
    # Bug 8 fix: call load_telemetry_data() ONCE and reuse
    ...
    # Sidebar: channel list + inspection panel
    with st.sidebar:
        st.markdown("""
        ...
        for ch_id, ch_info in channels.items():
            ...
            st.markdown(f"""
            <div class="mg-nav-item {active_class}" style="cursor: pointer; margin-bottom: 4px;">
                <div style="flex: 1;">
        ...
        ch_info_selected = channels[st.session_state.selected_channel]
        current_val = ts_data[st.session_state.selected_channel][-1]

        st.markdown(f"""
        <div style="margin-bottom: 16px;">
            <div class="label-caps" style="margin-bottom: 8px; color: var(--color-on-surface-variant);">Current Value</div>
            <div class="mg-data-mono-lg" style="color: var(--color-primary);">{current_val:.1f}<span class="mg-headline-md" style="color: var(--color-on-surface-variant); margin-left: 8px;">{ch_info_selected['unit']}</span></div>
    ...
    # Main chart area — Bug 8 fix: render directly (no unused col variables)
    ch_id = st.session_state.selected_channel
    values = ts_data[ch_id]
